...
class TicTacToeBoard:
    ...
    def make_move_second(self, row_: str, col: str) -> object:
        ...
        if self.field[row][col] != '[ ]':
            print(f'Cell {chr(row + ord("a")).upper()}{col + 1} already in use')
            return False
        else:
    ...
    def make_move(self, row: int, col: int) -> object:
        ...
        if self.field[row_][col_] != '[ ]':
            self.turn *= -1
            return f'Cell {chr(row + ord("a")).upper()}{col + 1} already in use'
        else:
    ...
    def run(self):
        while True:
            os.system('cls||clear')
            self.show_field()
            print(f'Turn {"first" if self.turn > 0 else "second"} player')
            while True:
                row = input('Enter value of row: ')
                col = input('Enter value of col: ')
            ...
            if self.check_winner() or self.check_field() == 'D':
                if self.check_field() == 'D':
                    ...
                    print(f'Congratulation! {"First" if self.turn > 0 else "Second"} player winner!')
                quest = input('Try Again?: Y/n ')
                if quest.lower() == 'y':
                    self.new_game()
                else:
                    break
